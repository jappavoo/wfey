...
for ((i = 0 ; i < ${times_to_run} ; i++ )); do
	...
	for events in "${eventrate[@]}"; do
		...
		for sources in "${sourceCPUs[@]}"; do
		    for perc in "${memperc[@]}"; do
			echo "$i: ./scripts/run_wfey.sh $config -e $events -p $eventproc -s $sources -w $perc"
			taskset -c ${runnercpu} ./scripts/run_wfey.sh $config -e $events -p $eventproc -s $sources -w $perc
